...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription('GLL DEV by Clutch - Tournament Management Bot')
            .setThumbnail(interaction.client.user.displayAvatarURL());

        if (isAdmin) {
            embed.addFields(
                { name: 'Admin Commands', value: 'Here are the commands available to administrators:' },
                { name: '/create_tournament', value: 'Start the process of creating a new tournament' },
                { name: '/seed', value: 'Manually set or adjust seedings for a tournament' },
                { name: '/start', value: 'Start a tournament, generating the initial matchups' },
    ...
    // No handleInteraction method needed for help command
};